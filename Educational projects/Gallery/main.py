...
def addartist():
    ...
    cursor.execute("""INSERT INTO Artists (name, address, town, country, postcode) VALUES (?, ?, ?, ?, ?)""",
                   (newname, newaddress, newtown, newcountry, newpostcode))
    db.commit()
...
def searchartistoutput():
    ...
    for x in cursor.fetchall():
        ...
        for i in cursor.fetchall():
            newrecord = str(i[0]) + ', ' + str(i[1]) + ', ' + str(i[2]) + ', ' + str(i[3]) + ', ₤' + str(i[4]) + '\n'
            outputwindow.insert(END, newrecord)
...
def searchmediumoutput():
    ...
    cursor.execute("""SELECT Art.prieceid, Artists.name, Art.title, Art.medium,
     Art.price FROM Artists, Art WHERE Artists.artisid= Art.artistid AND Art.medium = ?""", [selectedmedium])
    for i in cursor.fetchall():
        newrecord = str(i[0]) + ', ' + str(i[1]) + ', ' + str(i[2]) + ', ' + str(i[3]) + ', ₤' + str(i[4]) + '\n'
        outputwindow.insert(END, newrecord)
        medium2.set("")
...
searchartist.place(x=1025, y=300, width=60, height=25)
